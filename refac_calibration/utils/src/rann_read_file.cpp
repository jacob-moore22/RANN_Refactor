...
void PairRANN::read_file(char* filename)
{
    ...
    if (fp == nullptr) {
        errorf(FLERR, "Cannot open RANN potential file");
    }
    ...
    while (eof == 0) {
        ...
        if (linev[0] == "atomtypes") {
            read_atom_types(line1v, filename, linenum);
        }
        else if (linev[0] == "mass") {
            read_mass(linev, line1v, filename, linenum);
        }
        else if (linev[0] == "fingerprintsperelement") {
            read_fpe(linev, line1v, filename, linenum);
        }
        else if (linev[0] == "fingerprints") {
            read_fingerprints(linev, line1v, filename, linenum);
        }
        else if (linev[0] == "fingerprintconstants") {
            read_fingerprint_constants(linev, line1v, filename, linenum);
        }
        else if (linev[0] == "networklayers") {
            read_network_layers(linev, line1v, filename, linenum);
        }
        else if (linev[0] == "layersize") {
            read_layer_size(linev, line1v, filename, linenum);
        }
        else if (linev[0] == "weight") {
            read_weight(linev, line1v, fp, filename, &linenum);
        }
        else if (linev[0] == "bias") {
            read_bias(linev, line1v, fp, filename, &linenum);
        }
        else if (linev[0] == "activationfunctions") {
            read_activation_functions(linev, line1v, filename, linenum);
        }
        else if (linev[0] == "screening") {
            read_screening(linev, line1v, filename, linenum);
        }
        else if (linev[0] == "stateequationsperelement") {
            read_eospe(linev, line1v, filename, linenum);
        }
        else if (linev[0] == "stateequations") {
            read_eos(linev, line1v, filename, linenum);
        }
        else if (linev[0] == "stateequationconstants") {
            read_eos_constants(linev, line1v, filename, linenum);
        }
        else if (linev[0] == "bundles") {
            read_bundles(linev, line1v, filename, linenum);
        }
        else if (linev[0] == "bundleinput") {
            read_bundle_input(linev, line1v, filename, linenum);
        }
        else if (linev[0] == "bundleoutput") {
            read_bundle_output(linev, line1v, filename, linenum);
        }
        else if (linev[0] == "bundleid") {
            read_bundle_id(linev, line1v, filename, linenum);
        }
        else if (linev[0] == "calibrationparameters") {
            if (~is_lammps) {
                read_parameters(linev, line1v, fp, filename, &linenum, linetemp);
            }
        }
        else{
            errorf(filename, linenum - 1, "Could not understand file syntax: unknown keyword");
        }
        ptr = fgets(linetemp, longline, fp);
        linenum++;
        strtemp = utils::trim_comment(linetemp);
...
void PairRANN::read_fingerprints(std::vector<std::string> line, std::vector<std::string> line1, char* filename, int linenum)
{
    ...
    nwords  = line.size();
    if (nelements == -1) {
        errorf(filename, linenum - 1, "atom types must be defined before fingerprints in potential file.");
    }
    ...
    for (i = 1; i < nwords; i++) {
        ...
        if (!found) {
            errorf(filename, linenum - 1, "fingerprint element not found in atom types");
        }
    }
    i = atomtypes[0];
    k = 0;
    ...
    while (k < nwords1) {
        ...
        if (fingerprints[i][i1]->n_body_type != nwords - 1) {
            errorf(filename, linenum, "invalid fingerprint for element combination");
        }
        k++;
...
void PairRANN::read_fingerprint_constants(std::vector<std::string> line, std::vector<std::string> line1, char* filename, int linenum)
{
    ...
    for (k = 0; k < fingerprintperelement(i); k++) {
        ...
        if (n_body_type != fingerprints[i][k]->n_body_type) {
            continue;
        }
        for (j = 0; j < n_body_type; j++) {
            if (fingerprints[i][k]->atomtypes[j] != atomtypes[j]) {
                break;
            }
            if (j == n_body_type - 1) {
    ...
    if (!found) {
        errorf(filename, linenum - 1, "cannot define constants for unknown fingerprint");
    }
...
{
    std::cout << "**** Inside PairRANN::read_network_layers ****" << std::endl;
    int i, j;
    if (nelements == -1) {
    ...
    for (i = 0; i < nelements; i++) {
        ...
        if (line[1].compare(elements[i]) == 0) {
            ...
            net(i).bundles          = DualCArray<int>(net(i).layers, "net.bundles");
...
void PairRANN::read_layer_size(std::vector<std::string> line, std::vector<std::string> line1, char* filename, int linenum)
{
    ...
    for (i = 0; i < nelements; i++) {
        if (line[1].compare(elements[i]) == 0) {
            if (net(i).layers == 0) {
                errorf(filename, linenum - 1, "networklayers for each atom type must be defined before the corresponding layer sizes.");
            }
            int j = utils::inumeric(filename, linenum, line[2], true, lmp);
            if (j >= net(i).layers || j < 0) {
...
{
    std::cout << "**** Inside PairRANN::read_bundles ****" << std::endl;
    int i;
    for (i = 0; i < nelements; i++) {
        if (line[1].compare(elements[i]) == 0) {
            if (net(i).layers == 0) {
            ...
            int j = utils::inumeric(filename, linenum, line[2], true, lmp);
            if (j >= net(i).layers || j < 0) {
                errorf(filename, linenum - 1, "invalid layer in bundles definition");
            }
            ;
            net(i).bundles(j) = utils::inumeric(filename, linenum, line1[0], true, lmp);
            
            std::cout << "net(i).bundles(j) = " << net(i).bundles(j) << std::endl;
...
void PairRANN::read_bundle_id(std::vector<std::string> line, std::vector<std::string> line1, char* filename, int linenum)
{
    ...
    for (i = 0; i < nelements; i++) {
        if (line[1].compare(elements[i]) == 0) {
            ...
            int j = utils::inumeric(filename, linenum, line[2], true, lmp);
            if (j >= net(i).layers || j < 0) {
                errorf(filename, linenum - 1, "invalid layer in bundle inputs definition");
            }
            ;
            int b = utils::inumeric(filename, linenum, line[3], true, lmp);
            if (b > net(i).bundles(j) || b < 0) {
                errorf(filename, linenum - 1, "invalid bundle in bundle inputs");
            }
            int id = utils::inumeric(filename, linenum, line1[0], true, lmp);
            if (id != 1 && id != 0) {
                errorf(filename, linenum - 1, "bundle id must be 1 or 0\n");
            }
...
{   
    std::cout << "**** Inside PairRANN::read_bundle_input ****" << std::endl;
    int i, nwords;
    for (i = 0; i < nelements; i++) {
        if (line[1].compare(elements[i]) == 0) {
            if (net(i).layers == 0) {
            ...
            std::cout << "net(i).bundleinput[j][b]" << net(i).bundleinput[j][b] << std::endl;
...
{
    std::cout<< "**** Inside PairRANN::read_weight **** " << std::endl;
    ...
    if (nwords == 3) {
        ...
        b = utils::inumeric(filename, *linenum, line[3], true, lmp);
    }
    for (l = 0; l < nelements; l++) {
        if (line[1].compare(elements[l]) == 0) {
            if (net(l).layers == 0) {
                errorf(filename, *linenum - 1, "networklayers must be defined before weights.");
            }
            i = utils::inumeric(filename, *linenum, line[2], true, lmp);
            if (i >= net(l).layers || i < 0) {
                errorf(filename, *linenum - 1, "invalid weight layer");
            }
            if (dimensiondefined[l][i] == false || dimensiondefined[l][i + 1] == false) {
                errorf(FLERR, "network layer sizes must be defined before corresponding weight");
            }
            if (bundle_inputdefined[l][i][b] == false && b != 0) {
                errorf(filename, *linenum - 1, "bundle inputs must be defined before weights");
            }
            if (bundle_outputdefined[l][i][b] == false && b != 0) {
                errorf(filename, *linenum - 1, "bundle outputs must be defined before weights");
            }
            ...
            std::cout<< "net(l).bundleW[i][b]  size = "<< ins * ops << std::endl;

            net(l).bundleW[i][b] = new double [ins * ops];
            net(l).freezeW[i][b] = new bool [ins * ops];
            for (j = 0; j < ins * ops; j++) {
                net(l).freezeW[i][b][j] = 0;
            }
            weightdefined[l][i][b] = true;
            nwords = line1.size();
            if (nwords != ins) {
                errorf(filename, *linenum - 1, "invalid weights per line");
            }
            for (k = 0; k < ins; k++) {
            ...
            for (j = 1; j < ops; j++) {
                ...
                if (ptr == nullptr) {
                    errorf(filename, *linenum - 1, "unexpected end of potential file!");
                }
                nwords = line1.size();
                if (nwords != ins) {
                    errorf(filename, *linenum - 1, "invalid weights per line");
...
void PairRANN::read_bias(std::vector<std::string> line, std::vector<std::string> line1, FILE* fp, char* filename, int* linenum)
{
    ...
    for (l = 0; l < nelements; l++) {
        if (line[1].compare(elements[l]) == 0) {
            if (net(l).layers == 0) {
                errorf(filename, *linenum - 1, "networklayers must be defined before biases.");
            }
            i = utils::inumeric(filename, *linenum, line[2], true, lmp);
            if (i >= net(l).layers || i < 0) {
                errorf(filename, *linenum - 1, "invalid bias layer");
            }
            if (dimensiondefined[l][i] == false) {
                errorf(filename, *linenum - 1, "network layer sizes must be defined before corresponding bias");
            }
            if (bundle_outputdefined[l][i][b] == false && b != 0) {
                errorf(filename, *linenum - 1, "bundle outputs must be defined before bias");
            }
            if (net(l).identitybundle[i][b]) {
            ...
            for (j = 1; j < ops; j++) {
                ptr = fgets(linetemp, longline, fp);
                if (ptr == nullptr) {
                    errorf(filename, *linenum, "unexpected end of potential file!");
                }
                (*linenum)++;
                line1 = tokenmaker(linetemp, ": ,\t_\n");
...
void PairRANN::read_activation_functions(std::vector<std::string> line, std::vector<std::string> line1, char* filename, int linenum)
{
    ...
    for (l = 0; l < nelements; l++) {
        if (line[1].compare(elements[l]) == 0) {
            i = utils::inumeric(filename, linenum, line[2], true, lmp);
            if (i >= net(l).layers || i < 0) {
                errorf(filename, linenum - 1, "invalid activation layer");
            }
            if (dimensiondefined[l][i + 1] == false) {
...
void PairRANN::read_eos(std::vector<std::string> line, std::vector<std::string> line1, char* filename, int linenum)
{
    ...
    nwords  = line.size();
    if (nelements == -1) {
        errorf(filename, linenum - 1, "atom types must be defined before state equations in potential file.");
    }
    atomtypes = new int[nwords - 1];
    for (i = 1; i < nwords; i++) {
        found = false;
        ...
        if (!found) {
            errorf(filename, linenum - 1, "state equation element not found in atom types");
        }
    }
    i = atomtypes[0];
    k = 0;
    ...
    while (k < nwords1) {
        ...
        if (state[i][i1]->n_body_type != nwords - 1) {
            errorf(filename, linenum, "invalid state equation for element combination");
        }
        k++;
...
void PairRANN::read_eos_constants(std::vector<std::string> line, std::vector<std::string> line1, char* filename, int linenum)
{
    ...
    for (i = 1; i <= n_body_type; i++) {
        ...
        if (!found) {
            errorf(filename, linenum - 1, "equation of state element not found in atom types");
        }
    }
    i     = atomtypes[0];
    found = false;
    for (k = 0; k < stateequationperelement(i); k++) {
        ...
        if (n_body_type != state[i][k]->n_body_type) {
            continue;
        }
        for (j = 0; j < n_body_type; j++) {
            if (state[i][k]->atomtypes[j] != atomtypes[j]) {
                break;
            }
            if (j == n_body_type - 1) {
    ...
    if (!found) {
        errorf(filename, linenum - 1, "cannot define constants for unknown equation of state");
    }
...
void PairRANN::read_screening(std::vector<std::string> line, std::vector<std::string> line1, char* filename, int linenum)
{
    ...
    if (nwords != 5) {
        errorf(filename, linenum - 1, "invalid screening command");
    }
    int n_body_type = 3;
    atomtypes = new int[n_body_type];
    for (i = 1; i <= n_body_type; i++) {
    ...
    if (line[4].compare("Cmin") == 0) {
        screening_min(i,j,k) = utils::numeric(filename, linenum, line1[0], true, lmp);
    }
    else if (line[4].compare("Cmax") == 0) {
        screening_max(i,j,k) = utils::numeric(filename, linenum, line1[0], true, lmp);
    }
    else{
        errorf(filename, linenum - 1, "unrecognized screening keyword");
    }
    delete[] atomtypes;
}
...
{   
    std::cout << "**** Inside PairRANN::check_potential ****" << std::endl;
    int i, j, k, l;
    if (nelements == -1) {
        errorf(FLERR, "elements not defined!");
    }
    for (i = 0; i <= nelements; i++) {
        if (i < nelements) {
            if (mass(i) < 0) {
                errorf(FLERR, "mass not defined");   // uninitialized mass
            }
        }
        if (net(i).layers == 0) {
            continue;
        ...
        net(i).maxlayer  = 0;
        net(i).sumlayers = 0;
        net(i).startI    = DualCArray<int>(net(i).layers,"net.startI");
        ...
        for (j = 0; j < net(i).layers - 1; j++) {
            ...
            for (int k = 0; k < net(i).bundles(j); k++) {
                ...
                if (net(i).identitybundle[j][k]) {
                    ...
                    if (net(i).bundleinputsize[j][k] != net(i).bundleoutputsize[j][k]) {
                        errorf(FLERR, "input and output of identity bundles must be equal size");
                    }
                }
                if (!weightdefined[i][j][k] && !is_lammps) {
                    create_random_weights(net(i).bundleinputsize[j][k], net(i).bundleoutputsize[j][k], i, j, k);                          // undefined weights
                }
                else if (!weightdefined[i][j][k]) {
            ...
            for (int k = 0; k < net(i).dimensions(j + 1); k++) {
                ...
                if (!foundinput) {
                    errorf(FLERR, "found neuron with no input\n");
                }
            }
        }
...
void PairRANN::create_identity_wb(int rows, int columns, int itype, int layer, int bundle)
{
    ...
    std::cout<< "net(itype).bundleW[layer][bundle] size = "<< rows * columns << std::endl;
