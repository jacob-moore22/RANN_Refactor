...
void PairRANN::read_parameters(std::vector<std::string> line, std::vector<std::string> line1, FILE* fp, char* filename, int* linenum, char* linetemp)
{
    ...
    // }
        ...
        if (nwords == 4) {
            ...
            b = strtol(line[4].c_str(), NULL, 10);
        }
        for (l = 0; l < nelements; l++) {
            if (line[2] == elements[l]) {
                if (net(l).layers == 0) {
                    errorf("networklayers must be defined before weights.");
                }
                i = strtol(line[3].c_str(), NULL, 10);
                if (i >= net(l).layers || i < 0) {
                    errorf("invalid weight layer");
                }
                if (dimensiondefined[l][i] == false || dimensiondefined[l][i + 1] == false) {
                    errorf("network layer sizes must be defined before corresponding weight");
                }
                if (bundle_inputdefined[l][i][b] == false && b != 0) {
                    errorf("bundle inputs must be defined before weights");
                }
                if (bundle_outputdefined[l][i][b] == false && b != 0) {
                    errorf("bundle outputs must be defined before weights");
                }
                if (net(l).identitybundle[i][b]) {
                ...
                nwords = line1.size();
                if (nwords != ins) {
                    errorf("invalid weights per line");
                }
                for (k = 0; k < ins; k++) {
                ...
                for (j = 1; j < ops; j++) {
                    ...
                    if (ptr == NULL) {
                        errorf("unexpected end of potential file!");
                    }
                    nwords = line1.size();
                    if (nwords != ins) {
                        errorf("invalid weights per line");
        ...
        for (l = 0; l < nelements; l++) {
            if (line[2] == elements[l]) {
                if (net(l).layers == 0) {
                    errorf("networklayers must be defined before biases.");
                }
                i = strtol(line[3].c_str(), NULL, 10);
                if (i >= net(l).layers || i < 0) {
                    errorf("invalid bias layer");
                }
                if (dimensiondefined[l][i] == false) {
                    errorf("network layer sizes must be defined before corresponding bias");
                }
                if (bundle_outputdefined[l][i][b] == false && b != 0) {
                    errorf("bundle outputs must be defined before bias");
                }
                if (net(l).identitybundle[i][b]) {
...
void PairRANN::allocate(const std::vector<std::string>& elementwords)
{
    ...
    screening_min = DualCArray<double>(nelements, nelements, nelements, "screening_min");
    screening_max = DualCArray<double>(nelements, nelements, nelements, "screening_max");

    for (i = 0; i < nelements; i++) {
        for (int j = 0; j < nelements; j++) {
            for (int k = 0; k < nelements; k++) {
                screening_min(i, j, k) = 0.8;      // default values. Custom values may be read from potential file later.
                screening_max(i, j, k) = 2.8;      // default values. Custom values may be read from potential file later.
            }
        }
    }
...
bool PairRANN::check_parameters()
{
    ...
    if (tolerance == 0.0) {
        errorf(FLERR, "tolerance not correctly initialized\n");
    }
    if (tolerance < 0.0 || max_epochs < 0 || regularizer < 0.0 || potential_output_freq < 0) {
        errorf(FLERR, "detected parameter with negative value which must be positive.\n");
    }
    if (targettype > 3 || targettype < 1) {
        errorf(FLERR, "targettype must be 1, 2, or 3.");
    }
    srand(seed);
    count = 0;
    ...
    for (itype = 0; itype < nelementsp; itype++) {
        for (layer = 0; layer < net(itype).layers - 1; layer++) {
            for (bundle = 0; bundle < net(itype).bundles(layer); bundle++) {
                if (net(itype).identitybundle[layer][bundle]) {
                    continue;
                }
    ...
    freezebeta = DualCArray<bool>(betalen, "freezebeta");
    for (itype = 0; itype < nelementsp; itype++) {
        for (layer = 0; layer < net(itype).layers - 1; layer++) {
            for (bundle = 0; bundle < net(itype).bundles(layer); bundle++) {
                if (net(itype).identitybundle[layer][bundle]) {
...
void PairRANN::read_dump_files()
{
    ...
    while ( (entry = readdir(folder)) )
    {
        ...
        nwords--;
        if (nwords != 5 && nwords != 11 && nwords != 6 && nwords != 12) {
            errorf(entry->d_name, 2, "dumpfile must contain 2nd line with timestep, energy, energy_weight, force_weight, snapshots\n");
        }
    ...
    while ((entry = readdir(folder))) {
        ...
        ptr = fgets(line, MAXLINE, fid);      // ITEM: TIMESTEP
        while (ptr != NULL) {
            if (strstr(line, "ITEM: TIMESTEP") == NULL) {
                errorf("invalid dump file line 1");
            }
            ...
            nwords--;
            if (nwords != 5 && nwords != 11 && nwords != 6 && nwords != 12) {
                errorf("error: dump file line 2 must contain 5 entries: timestep, energy, energy_weight, force_weight, snapshots");
            }
            ...
            if (nwords == 11) {
                ...
                if (norm < 1e-14) {
                    errorf("spinaxis cannot be zero\n");
                }
            ...
            if (strstr(line, "ITEM: NUMBER OF ATOMS") == NULL) {
                errorf("invalid dump file line 3");
            }
            ptr = fgets(line, MAXLINE, fid);          // natoms
            int natoms = strtol(line, NULL, 10);
            // printf("%d %d %f\n",file,timestep,sims[file].energy/natoms);
            if (file > 0) {
                sims[file].startI = sims[file - 1].startI + natoms * 3;
            }
            this->natoms += natoms;
            if (targettype == 1) {
                sims[file].energy_weight /= pow(natoms, inum_weight);
            }
            ...
            if (strstr(line, "ITEM: BOX BOUNDS") == NULL) {
                errorf("invalid dump file line 5");
            }
            double box[3][3];
            double origin[3];
            bool   cols[12];
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if (nwords != 3 && nwords != 2) {
                    errorf("invalid dump box definition");
                }
            ...
            for (int i = 2; i < nwords; i++) {
                if (strcmp(words[i], "type") == 0) {
                    colid = 0;
                }
                else if (strcmp(words[i], "x") == 0) {
                    colid = 1;
                }
                else if (strcmp(words[i], "y") == 0) {
                    colid = 2;
                }
                else if (strcmp(words[i], "z") == 0) {
                    colid = 3;
                }
                else if (strcmp(words[i], "fx") == 0) {
                    colid = 4;
                }
                else if (strcmp(words[i], "fy") == 0) {
                    colid = 5;
                }
                else if (strcmp(words[i], "fz") == 0) {
                    colid = 6;
                }
                else if (strcmp(words[i], "sx") == 0) {
                    colid = 7;
                }
                else if (strcmp(words[i], "sy") == 0) {
                    colid = 8;
                }
                else if (strcmp(words[i], "sz") == 0) {
                    colid = 9;
                }
                else if (strcmp(words[i], "c_eng") == 0) {
                    colid = 10;
                }
                else {
                    continue;
            ...
            for (int i = 0; i < 4; i++) {
                if (!cols[i]) {
                    errorf("dump file must include type, x, y, and z data columns (other recognized keywords are fx, fy, fz, sx, sy, sz)");
                }
            }
            bool doforce = false;
            bool dospin  = false;
            ...
            if (targettype > 1) {
                sims[file].total_ea = new double [natoms];
            }
            ...
            for (int i = 0; i < natoms; i++) {
                ...
                nwords1--;
                if (nwords1 != nwords - 2) {
                    errorf("incorrect number of data columns in dump file.");
                }
            ...
            file++;
            if (file > nsims) {
                errorf("Too many dump files found. Nsims is incorrect.\n");
            }
        }
        fclose(fid);
    }
...
void PairRANN::create_neighbor_lists()
{
    ...
    for (i = 0; i < nsims; i++) {
        ...
        xm = sqrt(box[1][1] * box[2][2] * box[1][1] * box[2][2] + box[0][1] * box[0][1] * box[2][2] * box[2][2] + (box[0][1] * box[1][2] - box[0][2] * box[1][1]) *
                (box[0][1] * box[1][2] - box[0][2] * box[1][1]));
            ...
            for (iy = -yb; iy <= yb; iy++) {
                for (iz = -zb; iz <= zb; iz++) {
                    ...
                    for (j = 0; j < natoms; j++) {
                        ...
                        if (px > cutmax + fxp + buffer || px < fxn - cutmax - buffer) {
                            continue;
                        }
                        if (py > cutmax + fyp + buffer || py < fyn - cutmax - buffer) {
                            continue;
                        }
                        if (pz > cutmax + fzp + buffer || pz < fzn - cutmax - buffer) {
                            continue;
                        }
                        x[count][0] = xe;
                        x[count][1] = ye;
                        ...
                        if (sims[i].spinspirals && sims[i].spins) {
                            ...
                            // spins[j][0]=1;
                            // spins[j][2]=0;
                            theta = sims[i].spinvec[0] * (ix * box[0][0] + iy * box[0][1] + iz * box[0][2]) + sims[i].spinvec[1] * (iy * box[1][1] + iz * box[1][2]) + sims[i].spinvec[2] * iz *
                                box[2][2];
        ...
        for (nn = 0; nn < nsims; nn++) {
            ...
            for (ii = 0; ii < sims[nn].inum; ii++) {
                ...
                for (j = 0; j < len; j++) {
                    ...
                    else if (fingerprints[itype][j]->spin == false && fingerprints[itype][j]->screen == true) {
                        fingerprints[itype][j]->compute_fingerprint(features, dfeaturesx, dfeaturesy, dfeaturesz, Sik, dSikx, dSiky, dSikz, dSijkx, dSijky, dSijkz, Bij, ii, nn, xn, yn, zn, tn,
                            jnum - 1, jl);
                    }
                    else if (fingerprints[itype][j]->spin == true && fingerprints[itype][j]->screen == false) {
                        fingerprints[itype][j]->compute_fingerprint(features, dfeaturesx, dfeaturesy, dfeaturesz, sx, sy, sz, sxx, sxy, sxz, syy, syz, szz, ii, nn, xn, yn, zn, tn, jnum - 1, jl);
                    }
                    else if (fingerprints[itype][j]->spin == true && fingerprints[itype][j]->screen == true) {
                        fingerprints[itype][j]->compute_fingerprint(features, dfeaturesx, dfeaturesy, dfeaturesz, sx, sy, sz, sxx, sxy, sxz, syy, syz, szz, Sik, dSikx, dSiky, dSikz, dSijkx, dSijky,
                            dSijkz, Bij, ii, nn, xn, yn, zn, tn, jnum - 1, jl);
                    }
                }
                itype = nelements;
                ...
                for (j = 0; j < len; j++) {
                    ...
                    else if (state[itype][j]->screen == true && state[itype][j]->spin == false) {
                        state[itype][j]->eos_function(&e, force, Sik, dSikx, dSiky, dSikz, dSijkx, dSijky, dSijkz, Bij, i, nn, xn, yn, zn, tn, jnum - 1, jl);
                    }
                    else if (state[itype][j]->screen == false && state[itype][j]->spin == true) {
                        state[itype][j]->eos_function(&e, force, fm, i, nn, xn, yn, zn, tn, jnum - 1, jl);
                    }
                    else if (state[itype][j]->screen == true && state[itype][j]->spin == true) {
                        state[itype][j]->eos_function(&e, force, fm, Sik, dSikx, dSiky, dSikz, dSijkx, dSijky, dSijkz, Bij, i, nn, xn, yn, zn, tn, jnum - 1, jl);
                    }
                }
                itype = nelements;
                len   = stateequationperelement(itype);
                ...
                sims[nn].state_e += e;
                if (debug_level2_freq > 0) {
                    sims[nn].state_ea[ii] = e;
                }
                if (targettype > 1) {
                    sims[nn].total_ea[ii] -= e;
                }
            }
            clock_t end = clock();
...
///
/////////////////////////////////////////////////////////////////////////////
void PairRANN::unnormalize_net(CArray<NNarchitecture> net_out)
{
...
///
/////////////////////////////////////////////////////////////////////////////
void PairRANN::normalize_net(CArray<NNarchitecture> net_out)
{
...
///
/////////////////////////////////////////////////////////////////////////////
void PairRANN::copy_network(CArray<NNarchitecture> net_old, CArray<NNarchitecture> net_new)
{
    ...
    for (i = 0; i < nelementsp; i++) {
        ...
        if (net_new(i).layers > 0) {
            ...
            net_new(i).startI     = DualCArray<int>(net_new(i).layers, "net.startI");
            ...
            net_new(i).bundles = DualCArray<int>(net_new(i).layers - 1, "net.bundles");
...
void PairRANN::levenburg_marquardt_ch()
{
    ...
    jlen2 = jlen;
    if (doregularizer) {
        jlen += betalen;                  // do not regulate last bias
    }
    jlen1 = jlen;
...
//	double *Jp1 = J1;
    ...
    energy_fit /= jlen2;
    if (doforces) {
        for (i = nsimr; i < nsimr + natoms * 3; i++) {
            force_fit += tp[i] * tp[i];
        }
        force_fit /= natomsr * 3;
    }
    ...
    if (fid == NULL) {
        errorf("couldn't open log file!");
    }
    if (doforces) {
        sprintf(line, "iter: %d, evals: %d, e_err: %.10e, e1: %.10e, ev_err %.10e, f_err: %.10e, fv_err %.10e, r_err: %.10e, lambda: %.10e\n", iter, counter, energy_fit, energy_fit1, energy_fitv,
            force_fit, force_fitv, reg_fit, lambda);
    }
    else{
    ...
    while (iter < max_epochs) {
        if (goodstep) {
            ...
            double adexp = 0.1;
            if (adaptive_regularizer) {
                regularizer *= sqrt(initial_reg / reg_fit * energy_fit);
            }
            if (regularizer < 1e-8) {
                doregularizer = false;
                jlen -= betalen;
...
#pragma omp parallel
    ...
    while (iter < max_epochs) {
        ...
        if (alpha_max < alpha_min * 3) {
            alpha_max = alpha_min * 3;
        }
...
void PairRANN::bfgs()
{
    ...
    while (iter < max_epochs) {
        ...
        if (alpha_max < alpha_min * 5) {
            alpha_max = alpha_min * 5;
        }
        int nIter = ceil(-2.078087 * log(tolerance / abs(4 * alpha_min)));
        // int nIter = 5;
        x1 = (R * alpha_min + C * alpha_max) / Gmag;
...
///
/////////////////////////////////////////////////////////////////////////////
void PairRANN::flatten_beta(CArray<NNarchitecture> net, double* beta)
{
...
///
/////////////////////////////////////////////////////////////////////////////
void PairRANN::unflatten_beta(CArray<NNarchitecture> net, double* beta)
{
...
void PairRANN::get_per_atom_energy(double** energies, int* s, int sn, CArray<NNarchitecture> net)
{
...
void PairRANN::propagateforwardspin(double* energy, double** force, double** fm, double** hm, int ii, int jnum, int itype, double* features, double* dfeaturesx, double* dfeaturesy, double* dfeaturesz,
    double* sx, double* sy, double* sz, double* sxx, double* sxy, double* sxz, double* syy, double* syz, double* szz, int* jl, int nn, CArray<NNarchitecture> net)
{
        ...
        for (i = 0; i < nelements; i++) {
            for (j = 0; j < net_out(i).layers - 1; j++) {
                for (int k = 0; k < net_out(i).bundles(j); k++) {
                    if (net_out(i).identitybundle[j][k]) {
                        continue;
                    }
    ...
    // #pragma omp parallel
    {
        ...
        for (kk = 0; kk < jnum; kk++) { // outer sum over k in accordance with source, some others reorder to outer sum over jj
            ...
            for (jj = 0; jj < jnum; jj++) {
                ...
                if (rik + rjk - rij < 1e-13) {
                    continue;
                }                                        // bond angle > 90 degrees
                if (rik + rij - rjk < 1e-13) {
                    continue;
                }                                        // bond angle > 90 degrees
                double Cmax  = screening_max(itype, jtype, ktype);
                double Cmin  = screening_min(itype, jtype, ktype);
