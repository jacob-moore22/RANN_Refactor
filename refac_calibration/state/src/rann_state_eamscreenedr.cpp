...
void State_eamscreenedr::init(int* i, int _id)
{
    ...
    for (int j = 0; j < n_body_type; j++) {
        atomtypes[j] = i[j];
        if (i[j] != pair->nelements) {
            pair->errorf(FLERR, "State equation EAM is designed to use with all_all atom types only!");
        }
    }
    id = _id;
    ...
    double* zn, int* tn, int jnum, int* jl)
{
    ...
    if (rho > 0.0) {
        ...
        for (j = 0; j < jnum; j++) {
            ...
            if (atomtypes[1] != nelements && atomtypes[1] != tn[j]) {
                continue;
            }
            rsq = xn[j] * xn[j] + yn[j] * yn[j] + zn[j] * zn[j];
            if (rsq > rc[i][tn[j]] * rc[i][tn[j]]) {
...
bool State_eamscreenedr::parse_values(std::string constant, std::vector<std::string> line1)
{
    ...
    if (constant.compare("ec") == 0) {
        count = 0;
        if (nwords != (len1)) {
            pair->errorf(FLERR, "too many or too few ec values");
        }
        ...
        if (nwords != len1) {
            pair->errorf(FLERR, "too many or too few re values");
        }
        count = 0;
        for (l = 0; l < nelements; l++) {
            for (ll = 0; ll < nelements; ll++) {
        ...
        if (nwords != len1) {
            pair->errorf(FLERR, "too many or too few alpha values");
        }
        count = 0;
        for (l = 0; l < nelements; l++) {
            for (ll = 0; ll < nelements; ll++) {
        ...
        if (nwords != len1) {
            pair->errorf(FLERR, "too many or too few delta values");
        }
        count = 0;
        for (l = 0; l < nelements; l++) {
            for (ll = 0; ll < nelements; ll++) {
        ...
        if (nwords > pair->nelements) {
            pair->errorf(FLERR, "too many cweight values");
        }
        count = 0;
        for (l = 0; l < nwords; l++) {
        ...
        count = 0;
        if (nwords != len1) {
            pair->errorf(FLERR, "too many or too few dr values");
        }
        for (l = 0; l < nelements; l++) {
            for (ll = 0; ll < nelements; ll++) {
        ...
        if (nwords > pair->nelements) {
            pair->errorf(FLERR, "too many beta values");
        }
        for (l = 0; l < nwords; l++) {
        ...
        if (nwords > pair->nelements) {
            pair->errorf(FLERR, "too many Asub values");
        }
        for (l = 0; l < nwords; l++) {
        ...
        count = 0;
        if (nwords != nelements * nelements) {
            pair->errorf(FLERR, "too many or too few lattice values");
        }
        for (l = 0; l < nelements; l++) {
            for (ll = 0; ll < nelements; ll++) {
                ...
                if (l == ll && lat[l][ll]->n_body_type == 2) {
                    pair->errorf(FLERR, "tried to use binary lattice with one element!");
                }
                if (l != ll && lat[l][ll]->n_body_type == 1) {
                    pair->errorf(FLERR, "tried to use unary lattice with two elements!");
                }
                ...
                if (l != ll) {
                    ...
                    if (lat[ll][l]->empty == false) {
                        pair->errorf(FLERR, "multiple definitions of lattice");
                    }
                    delete lat[ll][l];
        ...
        pair->errorf(FLERR, "Undefined value for eamscreenedr equation of state");
    }
    bool finished = true;
    for (l = 0; l < nelements; l++) {
        if (Asub[l] == 0) {
...
{
    return -ec[i][j] * (1 + alpha[i][j] * (r / re[i][j] - 1) + delta[i][j] * alpha[i][j] * alpha[i][j] * alpha[i][j] * (r / re[i][j] - 1) * (r / re[i][j] - 1) * (r / re[i][j] - 1)) *
           exp(-alpha[i][j] * (r / re[i][j] - 1));
}
...
{
    return -ec[i][j] * exp(-alpha[i][j] * (r / re[i][j] - 1)) *
           (alpha[i][j] / re[i][j] + 2 * delta[i][j] * alpha[i][j] * alpha[i][j] / re[i][j] * (r / re[i][j] - 1) + 3 * delta[i][j] * alpha[i][j] * alpha[i][j] * alpha[i][j] / re[i][j] *
           (r / re[i][j] - 1) * (r / re[i][j] - 1) +
           (1 + alpha[i][j] * (r / re[i][j] - 1) + delta[i][j] * alpha[i][j] * alpha[i][j] * alpha[i][j] * (r / re[i][j] - 1) * (r / re[i][j] - 1) * (r / re[i][j] - 1)) * -alpha[i][j] / re[i][j]);
}
