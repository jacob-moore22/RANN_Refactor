...
bool State_covalent::parse_values(std::string constant, std::vector<std::string> line1)
{
    ...
    if (constant.compare("Ec") == 0) {
        if (nwords > pair->nelements) {
            pair->errorf(FLERR, "too many Ec values");
        }
        for (l = 0; l < nwords; l++) {
        ...
        if (nwords > pair->nelements) {
            pair->errorf(FLERR, "too many a values");
        }
        for (l = 0; l < nwords; l++) {
        ...
        if (nwords > pair->nelements) {
            pair->errorf(FLERR, "too many b values");
        }
        for (l = 0; l < nwords; l++) {
        ...
        if (nwords > pair->nelements) {
            pair->errorf(FLERR, "too many c values");
        }
        for (l = 0; l < nwords; l++) {
        ...
        pair->errorf(FLERR, "Undefined value for covalent equation of state");
    }
    // allow undefined delta, (default = 0)
    if (a != 0 && rc != 0 && b != 0 && dr != 0 && Ec[0] != 0) {
        return true;
    }
    return false;
}
