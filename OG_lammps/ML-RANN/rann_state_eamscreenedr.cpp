...
double State_eamscreenedr::get_drose(double r,int i,int j){
  return -ec[i][j]*exp(-alpha[i][j]*(r/re[i][j]-1))*(alpha[i][j]/re[i][j]+3*delta[i][j]*alpha[i][j]*alpha[i][j]*alpha[i][j]/re[i][j]*(r/re[i][j]-1)*(r/re[i][j]-1)+(1+alpha[i][j]*(r/re[i][j]-1)+delta[i][j]*alpha[i][j]*alpha[i][j]*alpha[i][j]*(r/re[i][j]-1)*(r/re[i][j]-1)*(r/re[i][j]-1))*-alpha[i][j]/re[i][j]);
}
...
double State_eamscreenedr::interpolate(double *table,double r1){
  ...
  double res = pair->res;
  int m1 = (int)(r1+0.01);
  if (m1>(res+2) || m1<1) {pair->errorf(FLERR,"invalid neighbor radius!");}
