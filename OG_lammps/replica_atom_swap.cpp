...
ReplicaAtomSwap::ReplicaAtomSwap(LAMMPS *lmp) : Command(lmp) {
    type_list = new int[2];
}

/* ---------------------------------------------------------------------- */

ReplicaAtomSwap::~ReplicaAtomSwap()
{
  MPI_Comm_free(&roots);
...
void ReplicaAtomSwap::command(int narg, char **arg)
{
  ...
  type_list[0] = utils::numeric(FLERR, arg[7], false, lmp);
  type_list[1] = utils::numeric(FLERR, arg[8], false, lmp);
  nswaptypes = 2;
  ...
  if ((!utils::strmatch(modify->fix[whichfix]->style,"^nvt")) &&
    ...
    error->universe_all(FLERR,"Replica atom swap temperature fix is not supported");
  ...
  for (int iswap = 0; iswap < nswaps; iswap++) {
   ...
   // printf("l5 %d\n",me_universe);
    for (int i = 0; i < 10; i++) nsuccess += attempt_swap();
    //printf("l6 %d\n",me_universe);
...
void ReplicaAtomSwap::print_status()
{
...
int ReplicaAtomSwap::pick_i_swap_atom(int iwhichglobal)
{
  int i = -1;
...
int ReplicaAtomSwap::pick_j_swap_atom(int jwhichglobal)
{
  int j = -1;
...
double ReplicaAtomSwap::energy_full()
{
  int eflag = 1;
  int vflag = 0;
...
int ReplicaAtomSwap::attempt_swap()
{
  int swap=0;
  //printf("a0 %d\n",me_universe);
  //
  ...
  //fprintf(universe->uscreen,"i: %d, j: %d, partner: %d\n",i,j,partner);
  int icheck,jcheck;
  MPI_Allreduce(&i, &icheck, 1, MPI_INT, MPI_SUM, world);
  MPI_Allreduce(&j, &jcheck, 1, MPI_INT, MPI_SUM, world);
  int check=0;
  if (universe->iworld < partner_root && icheck > -universe->procs_per_world[universe->iworld]){
    check = 1;
  }
  else if (universe->iworld > partner_root && jcheck > -universe->procs_per_world[universe->iworld]){
    check = 1;
  }
  int *checks;
  checks = new int[nworlds];
  ...
  if (j >= 0 && universe->iworld > partner_root) {
    atom->type[j] = itype;
...
//  if (unequal_cutoffs) {
  ...
  if (partner != -1) {
    if (me_universe > partner)
      MPI_Send(&energy_after,1,MPI_DOUBLE,partner,2,universe->uworld);
    else
      MPI_Recv(&energy_after_partner,1,MPI_DOUBLE,partner,2,universe->uworld,MPI_STATUS_IGNORE);
   ...
   // printf("a6 %d\n",me_universe);
    ...
    if (me_universe < partner){
      if (random_equal->uniform() < exp(beta * (energy_before+energy_stored_partner - energy_after-energy_after_partner))) {
        swap = 1;
      }
    }
   // printf("a7 %d\n",me_universe);
    ...
    if (me_universe < partner)
      MPI_Send(&swap,1,MPI_INT,partner,3,universe->uworld);
    else
      MPI_Recv(&swap,1,MPI_INT,partner,3,universe->uworld,MPI_STATUS_IGNORE);
  }
  MPI_Barrier(universe->uworld);
  //printf("a8 %d\n",me_universe);
  MPI_Bcast(&swap,1,MPI_INT,0,world);
...
void ReplicaAtomSwap::update_swap_atoms_list()
{
  ...
  niswap_local = 0;
  njswap_local = 0;
  int nswap = niswap+njswap;
  if (partner!=-1){
    if (me_universe<partner){
      if (random_equal->uniform()*nswap<niswap){
        int temp = type_list[0];
        type_list[0]=type_list[1];
        type_list[1]=temp;
      }
      MPI_Send(&type_list[0],1,MPI_INT,partner,4,universe->uworld);
    }
    else {
      MPI_Recv(&type_list[0],1,MPI_INT,partner,4,universe->uworld,MPI_STATUS_IGNORE);
    }
    if (me_universe<partner){
  ...
  MPI_Bcast(&type_list[0],1,MPI_INT,0,world);
  ...
  //printf("s4: me_un: %d,me: %d locali: %d i: %d j: %d jlocal: %d ibefore: %d jbefore: %d\n",me_universe,me,niswap_local,niswap,njswap,njswap_local,niswap_before,njswap_before);
}
