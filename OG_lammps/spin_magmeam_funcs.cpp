...
//
double
MAGMEAM::G_gam(const double gamma, const int ibar, int& errorflag) const
{
...
//
double
MAGMEAM::dG_gam(const double gamma, const int ibar, double& dG, double& ddG) const
{
  double gsmooth_switchpoint;
  double G;
  ...
  switch (ibar) {
	...
	ddG = gsmooth_factor * (2.0 + gsmooth_factor) * G / (4.0 * gamma * gamma);
        return G;
      } else {
        G = sqrt(1.0 + gamma);
        dG = 1.0 / (2.0 * G);
...
//
double
MAGMEAM::zbl(const double r, const int z1, const int z2)
{
  int i;
...
//
double
MAGMEAM::embedding(const double A, const double B, const double Ec, const double rhobar, double& dF, double& ddF) const
{
...
//
double
MAGMEAM::erose(const double r, const double re, const double alpha, const double Ec, const double repuls,
...
//
void
MAGMEAM::get_shpfcn(const lattice_t latt, const double sthe, const double cthe, double (&s)[3])
{
  switch (latt) {
    case FCC:
    case BCC:
...
//
int
MAGMEAM::get_Zij(const lattice_t latt)
{
  switch (latt) {
    case FCC:
      return 12;
...
//
int
MAGMEAM::get_Zij2(const lattice_t latt, const double cmin, const double cmax,
...
int
MAGMEAM::get_Zij2_b2nn(const lattice_t latt, const double cmin, const double cmax, double &S)
{
