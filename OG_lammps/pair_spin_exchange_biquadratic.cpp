...
void PairSpinExchangeBiquadratic::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	fmdsi[kk] = 0.0;
      ...
      if (rsq <= local_cut2) {
        compute_exchange(i,j,rsq,fmi,fmdsi,spi,spj);
...
void PairSpinExchangeBiquadratic::compute_single_pair(int ii, double fmi[3], double fmdsi[6])
{
  int *type = atom->type;
  double **x = atom->x;
  double **sp = atom->sp;
  ...
  if (locflag == 1) {
    ...
    for (int jj = 0; jj < jnum; jj++) {
      ...
      if (rsq <= local_cut2) {
        compute_exchange(ii,j,rsq,fmi,fmdsi,spi,spj);
      }
    }
  }
}
...
void PairSpinExchangeBiquadratic::compute_exchange(int i, int j, double rsq,
    double fmi[3], double fmdsi[6], double spi[3], double spj[3])
{
  int *type = atom->type;
  int itype,jtype;
  int ii,jj;
  ...
  for(ii = 0; ii < 3; ii++){
    for(jj = ii; jj < 3; jj++){
      fmdsi[count] += 2.0*Kex*spj[ii]*spj[jj];
      count += 1;
    }
  }
}
