...
void FixNVESpin::ComputeInteractionsSpin(int i)
{
  ...
  fmdsi[0] = fmdsi[1] = fmdsi[2] = fmdsi[3] = fmdsi[4] = fmdsi[5] = 0.0;
  // update magnetic pair interactions
  if (pair_spin_flag) {
    for (int k = 0; k < npairspin; k++) {
      spin_pairs[k]->compute_single_pair(i,fmi, fmdsi);
    }
  }
  ...
  if (setforce_spin_flag) {
    locksetforcespin->single_setforce_spin(i,fmi,fmdsi);
  }
...
void FixNVESpin::AdvanceSingleSpin(int i)
{
  ...
  double **fm = atom->fm;
  double **fmds = atom->fmds;  
  double fm2,energy,dts2,scale,msq;
  double cp[3],g[3],dwdt[3];
  ...
  dwdt[0] = (fmds[i][0]*cp[0] + fmds[i][1]*cp[1] + fmds[i][2]*cp[2]);
  dwdt[1] = (fmds[i][1]*cp[0] + fmds[i][3]*cp[1] + fmds[i][4]*cp[2]);
  dwdt[2] = (fmds[i][2]*cp[0] + fmds[i][4]*cp[1] + fmds[i][5]*cp[2]);

  g[0] += (dwdt[1]*sp[i][2] - dwdt[2]*sp[i][1])*0.5*dts2;
  g[1] += (dwdt[2]*sp[i][0] - dwdt[0]*sp[i][2])*0.5*dts2;
  g[2] += (dwdt[0]*sp[i][1] - dwdt[1]*sp[i][0])*0.5*dts2;
